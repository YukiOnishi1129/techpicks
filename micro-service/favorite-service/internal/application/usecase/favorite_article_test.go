...
func Test_UseCase_DeleteFavoriteArticle(t *testing.T) {
	t.Parallel()
	...
	tests := map[string]struct {
		recordFavoriteArticles       []entity.FavoriteArticle
		recordFavoriteArticleFolders []entity.FavoriteArticleFolder
		arg                          *fpb.DeleteFavoriteArticleRequest
		wantRecordFavoriteArticles   entity.FavoriteArticleSlice
		wantErrMsg                   string
	}{
		"success": {
			recordFavoriteArticles: []entity.FavoriteArticle{
				...
				},
				{
					...
					ArticleURL:              "article_url2",
					PlatformID:              null.String{String: platformID1, Valid: true},
					PublishedAt:             null.TimeFrom(time.Unix(publishedAt, 0)),
					AuthorName:              null.String{String: "author_name2", Valid: true},
					Tags:                    null.String{String: "tags2", Valid: true},
					PlatformName:            "platform_name2",
					PlatformURL:             "platform_url2",
					PlatformFaviconURL:      "platform_favicon_url2",
					IsEng:                   true,
					IsPrivate:               false,
				},
			},
		...
		"Fail: no target favorite article data": {
			recordFavoriteArticles: []entity.FavoriteArticle{
				{
					ID:                      faID1.String(),
					FavoriteArticleFolderID: fafID1.String(),
			...
			wantErrMsg: "favorite article not found",
		},
	}
	...
	for name, tt := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			_, err = testFavoriteUsecase.DeleteFavoriteArticle(ctx, tt.arg)
			if err != nil {
				if tt.wantErrMsg == "" {
					t.Error(err)
					return
