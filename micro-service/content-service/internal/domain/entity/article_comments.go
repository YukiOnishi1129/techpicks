...
// loaded structs of the objects. This is for an N-1 relationship.
func (articleCommentL) LoadArticle(ctx context.Context, e boil.ContextExecutor, singular bool, maybeArticleComment interface{}, mods queries.Applicator) error {
	var slice []*ArticleComment
	var object *ArticleComment
	...
	if singular {
		...
		foreign.R.ArticleComments = append(foreign.R.ArticleComments, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.ArticleID == foreign.ID {
				...
				foreign.R.ArticleComments = append(foreign.R.ArticleComments, local)
				break
			}
		}
	}
...
// SetArticle of the articleComment to the related item.
// Sets o.R.Article to related.
// Adds o to related.R.ArticleComments.
func (o *ArticleComment) SetArticle(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Article) error {
	var err error
	if insert {
		if err = related.Insert(ctx, exec, boil.Infer()); err != nil {
			return errors.Wrap(err, "failed to insert into foreign table")
	...
	if related.R == nil {
		related.R = &articleR{
			ArticleComments: ArticleCommentSlice{o},
		}
	} else {
		related.R.ArticleComments = append(related.R.ArticleComments, o)
	}
