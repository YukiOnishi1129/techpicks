		...
		t.Run(name, func(t *testing.T) {
			...
			if err != nil {
				...
				if diff := cmp.Diff(err.Error(), tt.wantErrMsg); diff != "" {
					t.Errorf("failed UpsertArticleComment (-got +want):\n%s", diff)
				}
				return
			}
...
func Test_DeleteArticleComment(t *testing.T) {
	t.Parallel()

	articleCommentID1, _ := uuid.NewRandom()
	articleCommentID2, _ := uuid.NewRandom()
	articleID, _ := uuid.NewRandom()
	publishedAt := time.Now().Add(-time.Hour * 24 * 7).Unix()
	mockPlatforms := mock.GetPlatformMock()
	mockProfiles := mock.GetProfileMock()
	...
	test := map[string]struct {
		recordArticles        []entity.Article
		recordArticleComments []entity.ArticleComment
		arg                   *cpb.DeleteArticleCommentRequest
		want                  *emptypb.Empty
		wantErrMsg            string
	}{
		"succeed to delete article comment": {
			recordArticles: []entity.Article{
				{
					ID: articleID.String(),
					PlatformID: null.String{
		...
		"failed to delete article comment as not exited article comment": {
			recordArticles: []entity.Article{
				{
					ID: articleID.String(),
					PlatformID: null.String{
			...
			arg: &cpb.DeleteArticleCommentRequest{
				UserId: userID1,
				Id:     articleCommentID2.String(),
			},
			want:       &emptypb.Empty{},
			wantErrMsg: "comment does not exist as delete target",
		},
	}
	...
	for name, tt := range test {
		...
		t.Run(name, func(t *testing.T) {
			...
			got, err := testContentUseCase.DeleteArticleComment(ctx, tt.arg)
			if err != nil {
				if tt.wantErrMsg == "" {
					t.Error(err)
					return
				...
				if diff := cmp.Diff(err.Error(), tt.wantErrMsg); diff != "" {
					t.Errorf("failed DeleteArticleComment (-got +want):\n%s", diff)
				}
				return
			}
