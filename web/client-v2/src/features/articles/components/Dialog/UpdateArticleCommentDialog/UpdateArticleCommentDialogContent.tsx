...
type UpdateArticleCommentDialogContentProps = {
...
export const UpdateArticleCommentDialogContent: FC<
  UpdateArticleCommentDialogContentProps
> = ({ data, articleId, articleTitle, onClose }) => {
  const { successToast, failToast } = useStatusToast();
  const [isPending, startTransition] = useTransition();

  const fragment = readFragment(UpdateArticleCommentDialogFragment, data);
  ...
  const onSubmit = useCallback(
    async (data: z.infer<typeof FormSchema>) => {
      startTransition(async () => {
        ...
        if (!user) {
          failToast({
            description: "Please login to update an article comment",
          });
      ...
      successToast({
        description: `edit comment at article title: '${articleTitle}'`,
      });
      onClose();
    },
    [
  ...
  return (
    <DialogContent onCloseAutoFocus={resetDialog}>
      <DialogHeader>
        <DialogTitle>{"Edit Article Comment"}</DialogTitle>
